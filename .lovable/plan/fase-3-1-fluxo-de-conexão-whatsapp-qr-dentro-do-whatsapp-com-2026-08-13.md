...
5. **Transição automática para "Conectado" só com identidade validada**: enquanto o modal estiver aberto, o estado real é verificado periodicamente. Ao detectar `open`, o servidor sincroniza `owner_jid` + `owner_number_digits` e compara com o número esperado do endpoint. O modal só fecha automaticamente, só mostra "Conectado" e só invalida o status como conexão válida quando as três condições forem booleano `true` explícito: `connected === true && identityKnown === true && identityMatchesEndpoint === true`. `null`, `undefined`, estado desconhecido ou qualquer valor diferente de `true` **não** é sucesso. Casos parciais mantêm o modal aberto com mensagem específica: `connected=true` e `identityKnown=false` → "Identidade não confirmada"; `identityMatchesEndpoint=false` → "O número conectado diverge do número deste endpoint". Em ambos, "Tornar ativo" continua bloqueado.
...
- **`setActiveEndpoint` — revalidação fresca no backend no momento do clique (Evolution)**: imediatamente antes de chamar `rotate_messaging_line_endpoint`, localizar a instância vinculada, consultar o estado REAL atual na Evolution, exigir `open/connected`, ressincronizar `owner_jid` + `owner_number_digits` a partir da resposta real do provider e exigir igualdade exata com o número normalizado do endpoint. Erros obrigatórios: `ACTIVATE_EVOLUTION_NOT_CONNECTED` (sessão desconectada), `ACTIVATE_EVOLUTION_IDENTITY_UNKNOWN` (identidade ausente), `ACTIVATE_EVOLUTION_IDENTITY_MISMATCH` (número divergente). Em qualquer uma dessas falhas: **não** alterar `active_endpoint_id` e **não** criar registro em `messaging_line_rotations` (a RPC nem é chamada). Meta/Twilio preservam as validações já aprovadas.

# Fase 3 — Migração corrigida (guardas endurecidas)

Duas correções aplicadas ao desenho aprovado, mais nada. Nada será executado antes da sua aprovação.

## Verificação prévia (read-only, feita agora)

- Nenhum endpoint WhatsApp tem `purpose` nulo hoje (24 endpoints: `commercial`, `customer_service`, `other`). Logo a cláusula "adoção quando o purpose legado é NULL" nasce sem efeito retroativo.
- Não existe nenhum endpoint com `purpose='support'` nem `vendor_personal`. `support` fica fora da whitelist de rotação (nenhum legado real para tolerar).
- Padrões atuais: Route `sales` das duas orgs aponta para endpoint `commercial`; Route `customer_service` aponta para endpoint `customer_service`. As novas guardas de compatibilidade são satisfeitas pelo estado atual — nenhuma rotação existente quebra.
- Endpoints `purpose='other'` (Twilio antigos) passam a ser inelegíveis como padrão de qualquer Route; nenhum deles é padrão hoje.

## Correção 1 — `provision_line_endpoint`: sem reclassificação silenciosa

Substitui a regra anterior (que só bloqueava quando havia vínculo ativo em outra Route):

- Endpoint existente com `purpose` **não nulo** e diferente de `p_purpose` → `PROVISION_ENDPOINT_PURPOSE_CONFLICT`, sempre, independentemente de vínculos.
- Adoção de `purpose` só quando o valor atual é **NULL**.
- `vendor_personal` reutilizado: se já tem `assigned_user_id` e é diferente do usuário aprovado → `PROVISION_ASSIGNED_USER_CONFLICT`. Nunca reatribui dono silenciosamente.
- Endpoint não-pessoal: `assigned_user_id` é forçado a `NULL` (não carrega dono residual).
- `provision_line_endpoint` continua **nunca** escrevendo `messaging_lines.active_endpoint_id`.

## Correção 2 — `rotate_messaging_line_endpoint`: compatibilidade + vínculo obrigatório

- Route `sales`: aceita como padrão **apenas** `purpose='commercial'`. `vendor_personal` recusado (`ROTATION_PERSONAL_NOT_ALLOWED`), `customer_service` recusado (`ROTATION_PURPOSE_LINE_MISMATCH`).
- Route `customer_service`: aceita **apenas** `purpose='customer_service'`.
- Exige vínculo **ativo pré-existente** em `messaging_line_endpoints` para aquela Route; se não houver → `ROTATION_ENDPOINT_NOT_LINKED`. A função deixa de criar/reativar vínculo (não pode contornar o provisionamento).
- Mantidos: admin-only, org/canal, endpoint ativo, `ROTATION_ENDPOINT_IN_USE`, log em `messaging_line_rotations`.

## SQL a executar (uma migração)
...
```text
EXISTING_ENDPOINT_PURPOSE_RECLASSIFICATION=BLOCKED
PERSONAL_OWNER_REASSIGNMENT=BLOCKED
PERSONAL_AS_ROUTE_DEFAULT=BLOCKED
SALES_ENDPOINT_ON_CUSTOMER_SERVICE_ROUTE=BLOCKED
CUSTOMER_SERVICE_ENDPOINT_ON_SALES_ROUTE=BLOCKED
ROTATION_REQUIRES_EXISTING_ROUTE_LINK=YES
PROVISION_CHANGES_ACTIVE_ENDPOINT=NO
EXISTING_ENDPOINTS_CHANGED=0
```

`EXISTING_ENDPOINTS_CHANGED=0` é estrutural: a migração só contém `CREATE OR REPLACE FUNCTION` e grants — nenhum DML sobre `communication_endpoints`, `messaging_lines` ou `messaging_line_endpoints`.

## Depois da migração (restante da Fase 3, já aprovado)

Edge `sales-route-operations` (status com `customer_service`, `provisionEndpoint` por destino), `meta-whatsapp-connect` (destino Pessoal + vínculo via RPC), UI com passo "Destino" + select de usuário, e o ensaio com ROLLBACK do relatório de conformidade.

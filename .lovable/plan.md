...
- SCHEMA_CHANGES_REQUIRED=sem novas tabelas/colunas. Só funções: `fn_message_threads_autofill_business_context` (`vendor_personal → 'sales'`), nova `fn_can_user_use_reply_endpoint`, `fn_guard_user_reply_endpoint` (endpoint pessoal só aceita grant do próprio dono), nova `provision_line_endpoint(...)` aceitando `sales|customer_service` + `p_assigned_user_id`, `rotate_messaging_line_endpoint` liberando `customer_service`

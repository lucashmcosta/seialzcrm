      ...
      {state === 'consulting' && (
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <Button disabled={busy} onClick={() => void controlTransfer('complete')}>
            <UserSwitch className="mr-2 h-4 w-4" /> Transferir agora
          </Button>
        </div>
      )}
      {state === 'with_customer' && (
        <div className="grid gap-2 sm:grid-cols-2">
          <Button variant="outline" disabled={busy} onClick={() => void controlTransfer('consult_again')}>
            <ArrowsLeftRight className="mr-2 h-4 w-4" /> Consultar novamente
          </Button>
          <Button variant="secondary" disabled={busy} onClick={() => void controlTransfer('cancel')}>Encerrar transferência</Button>
        </div>
      )}
      {busy && <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground"><SpinnerGap className="h-3 w-3 animate-spin" /> Processando…</div>}
    </div>
  );
}

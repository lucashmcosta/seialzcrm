...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_opportunities_org_status
  ON opportunities(organization_id, status)
  WHERE deleted_at IS NULL;

-- Opportunities: close_date for forecasting queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_opportunities_org_close_date
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tag_assignments_entity
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pipeline_stages_org_order
  ON pipeline_stages(organization_id, order_index);

-- Message thread reads: JOIN in rpc_list_message_threads
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_thread_reads_thread_user
  ON message_thread_reads(thread_id, user_id);

-- Contact memories: org + contact composite
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_contact_memories_org_contact
  ON contact_memories(organization_id, contact_id);

-- AI agent logs: contact and thread lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ai_agent_logs_contact
  ON ai_agent_logs(contact_id)
  WHERE contact_id IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ai_agent_logs_thread

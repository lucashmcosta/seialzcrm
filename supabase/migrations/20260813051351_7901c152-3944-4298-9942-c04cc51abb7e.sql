ALTER TABLE public.evolution_instances
  ...
  ADD COLUMN IF NOT EXISTS owner_number_digits text;

COMMENT ON COLUMN public.evolution_instances.owner_jid IS 'Identidade real da instancia (ownerJid) obtida SOMENTE da resposta real da Evolution API server-side. Proibido preencher por inferencia (instance_name, display_name, endpoints, input do frontend).';
COMMENT ON COLUMN public.evolution_instances.owner_number_digits IS 'Numero real da instancia normalizado (somente digitos), derivado SOMENTE da resposta real da Evolution API server-side.';
...
BEGIN
  ...
  -- 5. identidade do endpoint (read-only, sem escolha silenciosa)
...
REVOKE ALL ON FUNCTION public.provision_sales_endpoint(uuid,uuid,text,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.provision_sales_endpoint(uuid,uuid,text,text,text,text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.provision_sales_endpoint(uuid,uuid,text,text,text,text) TO service_role;
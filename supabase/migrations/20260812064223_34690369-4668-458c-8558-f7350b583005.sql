-- 1) Replay puro do estado operacional do winner
CREATE OR REPLACE FUNCTION public.fn_replay_sales_merge_state(
  p_baseline jsonb,
  p_audit_ids uuid[],
  OUT o_status text,
  OUT o_assignee uuid,
  OUT o_assigned_at timestamptz,
  OUT o_resolved_at timestamptz
)
RETURNS record
LANGUAGE plpgsql
STABLE
...
REVOKE ALL ON FUNCTION public.fn_replay_sales_merge_state(jsonb, uuid[]) FROM PUBLIC, anon, authenticated;

-- 2) merge_sales_threads: marker SALES_V2, invariante de batch, bloqueio de audit legada, stamp de assignment_history
...
BEGIN
  ...
  -- 0a) auditoria legada (outra semantica) em QUALQUER papel impede o contrato SALES_V2
  IF EXISTS (
...
-- 3) unmerge_message_thread: caminho SALES_V2 com guard de deriva, stamp exato e replay por batch
...
BEGIN
  ...
  IF v_is_sales THEN
    ...
    SELECT x.winner_snapshot INTO v_baseline
     ...
     WHERE x.winner_thread_id = v_winner
       ...
       AND COALESCE(x.winner_snapshot->>'_merge_kind','') = 'SALES_V2'
     ORDER BY x.executed_at ASC, x.id ASC LIMIT 1;

    SELECT array_agg(x.id ORDER BY x.executed_at, x.id) INTO v_ids_expected
      FROM public.message_thread_merge_audit x
     WHERE x.winner_thread_id = v_winner
       AND x.batch_id = v_batch
       AND COALESCE(x.winner_snapshot->>'_merge_kind','') = 'SALES_V2'
  ...
  IF v_is_sales THEN
    SELECT array_agg(x.id ORDER BY x.executed_at, x.id) INTO v_ids_target
      FROM public.message_thread_merge_audit x
     WHERE x.winner_thread_id = v_winner
       AND x.batch_id = v_batch
       AND COALESCE(x.winner_snapshot->>'_merge_kind','') = 'SALES_V2'

-- ============================================================================
-- 1) FONTE UNICA DE AUTORIZACAO (interna: nao executavel por authenticated)
-- ============================================================================
CREATE OR REPLACE FUNCTION public.can_user_manage_integrations_in_org(
...
REVOKE ALL ON FUNCTION public.can_user_manage_integrations_in_org(uuid,uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.can_user_manage_integrations_in_org(uuid,uuid) FROM authenticated;
REVOKE ALL ON FUNCTION public.can_user_manage_integrations_in_org(uuid,uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.can_user_manage_integrations_in_org(uuid,uuid) TO service_role;

-- Mesma assinatura, mesmos grants, mesmo resultado: agora apenas delega.
-- SECURITY DEFINER => executa como owner, portanto pode chamar a interna.
CREATE OR REPLACE FUNCTION public.can_manage_integrations_in_org(_org_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $fn$
  SELECT public.can_user_manage_integrations_in_org(public.current_user_id(), _org_id);
$fn$;

-- ============================================================================
-- 2) CORE unico de provisionamento (regras identicas as de hoje).
-- Aditivos: autorizacao canonica parametrizada + prova de posse Twilio
-- verificada server-side, que SO conta com integracao twilio ativa da org.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.provision_line_endpoint_core(
  p_actor_override uuid,
...
DECLARE
  v_actor uuid := COALESCE(p_actor_override, public.current_user_id());
...
BEGIN
  -- UMA unica regra de autorizacao (canonica), para todos os caminhos.
  IF v_actor IS NULL OR p_organization_id IS NULL
     OR NOT public.can_user_manage_integrations_in_org(v_actor, p_organization_id) THEN
    RAISE EXCEPTION 'PROVISION_FORBIDDEN';
  END IF;
  -- Gate extra preservado apenas para o caminho atual (JWT do usuario).
  IF p_actor_override IS NULL
     AND NOT (p_organization_id = ANY (public.current_user_org_ids())) THEN
    RAISE EXCEPTION 'PROVISION_FORBIDDEN';
  END IF;
  ...
  ELSE
    ...
    -- ADITIVO: posse Twilio verificada server-side pela Edge.
    -- Ordem invertida: senderSid sozinho NAO torna o numero owned.
    -- Exige integracao twilio-whatsapp ATIVA da propria organizacao.
    IF NOT v_owned AND p_provider = 'twilio'
       AND p_sender_sid IS NOT NULL AND btrim(p_sender_sid) <> '' THEN
      SELECT oi.id INTO v_integration
        FROM public.organization_integrations oi
        JOIN public.admin_integrations ai ON ai.id = oi.integration_id
       WHERE oi.organization_id = p_organization_id
         AND ai.slug = 'twilio-whatsapp' AND oi.is_enabled = true
       LIMIT 1;
      IF v_integration IS NULL THEN
        RAISE EXCEPTION 'PROVISION_TWILIO_INTEGRATION_NOT_FOUND';
      END IF;
      v_owned := true;
    END IF;
  ...
  IF v_endpoint_id IS NOT NULL THEN
    ...
    UPDATE public.communication_endpoints
       SET is_active = true,
           ...
           sender_sid = COALESCE(NULLIF(btrim(COALESCE(p_sender_sid,'')),''), sender_sid),
           external_account_id = COALESCE(
             NULLIF(btrim(COALESCE(p_external_account_id,'')),''), external_account_id),
           organization_integration_id = CASE
             WHEN p_sender_sid IS NOT NULL THEN COALESCE(organization_integration_id, v_integration)
             ELSE organization_integration_id END,
           updated_at = now()
  ...
  ELSE
    ...
    INSERT INTO public.communication_endpoints (
      ...
      inbound_settings, sender_sid, external_account_id)
    VALUES (p_organization_id, v_integration, 'whatsapp', p_address,
      ...
      NULLIF(btrim(COALESCE(p_sender_sid,'')),''),
...
REVOKE ALL ON FUNCTION public.provision_line_endpoint_core(uuid,uuid,uuid,text,text,text,text,text,uuid,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.provision_line_endpoint_core(uuid,uuid,uuid,text,text,text,text,text,uuid,text,text)
  TO service_role;

-- ============================================================================
-- 3) RPC PUBLICA: MESMA assinatura, MESMOS defaults, MESMOS grants.
-- ============================================================================
...
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $fn$
BEGIN
  RETURN public.provision_line_endpoint_core(
    NULL, p_organization_id, p_line_id, p_provider, p_address, p_purpose,
    p_display_name, p_instance_name, p_assigned_user_id, NULL, NULL);
END $fn$;
...
-- ============================================================================
-- 4) ENTRADA ADICIONAL EXCLUSIVA DO TWILIO (apenas service_role).
-- ============================================================================
CREATE OR REPLACE FUNCTION public.provision_line_endpoint_twilio_verified(
  p_actor_user_id uuid, p_organization_id uuid, p_line_id uuid, p_address text,
  p_purpose text, p_sender_sid text, p_external_account_id text DEFAULT NULL,
  p_display_name text DEFAULT NULL, p_assigned_user_id uuid DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $fn$
BEGIN
  IF p_actor_user_id IS NULL THEN RAISE EXCEPTION 'PROVISION_ACTOR_REQUIRED'; END IF;
  IF p_sender_sid IS NULL OR btrim(p_sender_sid) = '' THEN
    RAISE EXCEPTION 'PROVISION_TWILIO_SENDER_PROOF_REQUIRED'; END IF;
  RETURN public.provision_line_endpoint_core(
    p_actor_user_id, p_organization_id, p_line_id, 'twilio', p_address, p_purpose,
    p_display_name, NULL, p_assigned_user_id, p_sender_sid, p_external_account_id);
END $fn$;

REVOKE ALL ON FUNCTION public.provision_line_endpoint_twilio_verified(uuid,uuid,uuid,text,text,text,text,text,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.provision_line_endpoint_twilio_verified(uuid,uuid,uuid,text,text,text,text,text,uuid)
  TO service_role;
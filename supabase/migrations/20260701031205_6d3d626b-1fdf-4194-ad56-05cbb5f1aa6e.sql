-- Fase 1: adicionar c.organization_id = p_organization_id no INNER JOIN contacts
-- em ambas as RPCs, para permitir uso de idx_contacts_org_lifecycle_customer
-- e eliminar Seq Scan em contacts. Filtro é logicamente redundante (contato
-- de outra org nunca casaria com t.contact_id de thread da org), apenas ajuda
-- o planner.
...
-- Hardening: fecha EXECUTE default para PUBLIC/anon e concede apenas a authenticated.
REVOKE ALL ON FUNCTION public.rpc_inbox_queue_counts(uuid, boolean, uuid, timestamptz, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.rpc_inbox_queue_counts(uuid, boolean, uuid, timestamptz, boolean) FROM anon;
GRANT EXECUTE ON FUNCTION public.rpc_inbox_queue_counts(uuid, boolean, uuid, timestamptz, boolean) TO authenticated;

REVOKE ALL ON FUNCTION public.rpc_list_inbox_threads(uuid, text, boolean, uuid, timestamptz, boolean, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.rpc_list_inbox_threads(uuid, text, boolean, uuid, timestamptz, boolean, integer) FROM anon;
GRANT EXECUTE ON FUNCTION public.rpc_list_inbox_threads(uuid, text, boolean, uuid, timestamptz, boolean, integer) TO authenticated;
-- Rebuild sales dashboard aggregation with optional explicit previous window.
-- DROP + CREATE (not CREATE OR REPLACE) to avoid creating an ambiguous overload.

DROP FUNCTION IF EXISTS public.get_sales_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text);
DROP FUNCTION IF EXISTS public.get_sales_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, text);

CREATE FUNCTION public.get_sales_dashboard_stats_core(
  ...
  p_tz text DEFAULT 'America/Sao_Paulo'::text,
  p_prev_from timestamptz DEFAULT NULL::timestamptz,
  p_prev_to timestamptz DEFAULT NULL::timestamptz,
  p_prev_from_day date DEFAULT NULL::date,
  p_prev_to_day date DEFAULT NULL::date
)
RETURNS json
LANGUAGE sql
STABLE
...
SELECT json_build_object(
  ...
  'prev_range', json_build_object(
    'from', (SELECT prev_from FROM b), 'to', (SELECT prev_to FROM b),
    'from_day', (SELECT prev_from_day FROM b), 'to_day', (SELECT prev_to_day FROM b),
    'explicit', (p_prev_from IS NOT NULL OR p_prev_from_day IS NOT NULL)
  ),
...
CREATE FUNCTION public.get_sales_dashboard_stats(
  p_organization_id uuid,
  p_from timestamptz,
  p_to timestamptz,
  p_from_day date,
...
BEGIN
  ...
  SELECT public.get_sales_dashboard_stats_core(
           ...
           p_prev_from, p_prev_to, p_prev_from_day, p_prev_to_day
         )
  INTO v_result;
...
-- Permissions: wrapper callable by authenticated; core stays private.
REVOKE ALL ON FUNCTION public.get_sales_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_sales_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_sales_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) TO authenticated, service_role;

REVOKE ALL ON FUNCTION public.get_sales_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_sales_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM anon;
REVOKE ALL ON FUNCTION public.get_sales_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_sales_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) TO service_role;
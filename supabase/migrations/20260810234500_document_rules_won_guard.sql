-- Fase 2 — Regras de documento para fechar a oportunidade (server-side, canônico).
-- Aditivo e seguro: document_rules default vazio => comportamento idêntico ao atual.
-- O guard de WON já é canônico (trigger fn_guard_opportunity_won_requirements_v1 -> este evaluator)
-- e só BLOQUEIA em mode='enforce'. Começamos em monitor (nenhuma org em enforce hoje).

alter table public.opportunity_close_policies
  add column if not exists document_rules jsonb not null default '{"version": 1, "rules": []}'::jsonb;

-- Estende o evaluator compartilhado: além de CPF/endereço/campos, checa documentos exigidos.
-- Regra: { document_type_id, effect:'require', when:null }. "when" reservado para o futuro (não usado agora).
-- Dono do documento = contato da oportunidade (document_types são orientados a contato hoje).
...
begin
  ...
  -- >>> Fase 2: regras de documento (exige documento por tipo no contato da oportunidade) <<<
  for v_field in
    select (r->>'document_type_id')
    from jsonb_array_elements(coalesce(v_policy.document_rules -> 'rules', '[]'::jsonb)) r
    where coalesce(r->>'effect', 'require') = 'require'
      and (r->>'document_type_id') is not null
  loop
    v_ok := v_op.contact_id is not null and exists (
      select 1 from public.documents d
      where d.organization_id = _organization_id
        and d.entity_type = 'contact'
        and d.entity_id = v_op.contact_id
        and d.document_type_id = v_field::uuid
        and d.deleted_at is null
    );
    if not v_ok then v_missing := array_append(v_missing, 'document:' || v_field); end if;
    v_items := v_items || jsonb_build_array(jsonb_build_object(
      'code', 'document:' || v_field,

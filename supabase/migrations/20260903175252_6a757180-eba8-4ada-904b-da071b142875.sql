DROP FUNCTION IF EXISTS public.get_home_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text);
DROP FUNCTION IF EXISTS public.get_home_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, boolean, uuid, text);

CREATE FUNCTION public.get_home_dashboard_stats_core(
...
WITH bounds AS (
  SELECT
    ...
    COALESCE(p_prev_from,     p_from - (p_to - p_from) - interval '1 millisecond')        AS prev_from,
    COALESCE(p_prev_to_day,   p_from_day)                                                 AS prev_to_day,
    COALESCE(p_prev_from_day,
      (((p_from - (p_to - p_from) - interval '1 millisecond') AT TIME ZONE p_tz)::date))  AS prev_from_day
),
effective_owner AS (
  SELECT CASE
...
REVOKE ALL ON FUNCTION public.get_home_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, boolean, uuid, text, timestamptz, timestamptz, date, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_home_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, boolean, uuid, text, timestamptz, timestamptz, date, date) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_home_dashboard_stats_core(uuid, timestamptz, timestamptz, date, date, uuid, boolean, uuid, text, timestamptz, timestamptz, date, date) TO service_role;

CREATE FUNCTION public.get_home_dashboard_stats(
  p_organization_id uuid,
  p_from timestamptz,
  p_to timestamptz,
  p_from_day date,
...
REVOKE ALL ON FUNCTION public.get_home_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_home_dashboard_stats(uuid, timestamptz, timestamptz, date, date, uuid, text, timestamptz, timestamptz, date, date) TO authenticated, service_role;
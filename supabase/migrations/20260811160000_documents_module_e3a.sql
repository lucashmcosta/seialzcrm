-- ============================================================
-- Módulo de Documentos — ETAPA 3a: regras de fechamento por SET (evaluator)
-- Projeto: qvmtzfvkhkhkhdpclzua
--
-- Reescreve SÓ a seção de documentos do evaluate_opportunity_close_internal_v1:
-- resolve UM set aplicável (maior priority cujo `when` bate; when null = default),
-- e para cada CÓDIGO em `required` checa presença OWNER-AWARE (contato vs oportunidade)
-- de um documento CORRENTE e NÃO INCOMPLETO. Demais checagens (CPF/endereço/campos/
-- custom fields) permanecem IDÊNTICAS. Guard/trigger de WON inalterado.
--
-- document_rules migra de {version:1, rules:[]} para {version:2, sets:[]} (sem mudança
-- de comportamento — vazio nas 2 orgs). Reversível (ver rollback).
-- ============================================================
...
declare
  ...
  v_value jsonb;
  -- Etapa 3: resolução de set/when + documentos owner-aware
  v_set jsonb;
  v_best_set jsonb;
  v_when jsonb;
  v_cond jsonb;
...
begin
  ...
  if coalesce(v_policy.require_cpf_verified, false) then
    ...
    v_fallback := not v_ok and public.is_valid_cpf(v_contact.cpf)
  ...
  -- >>> Etapa 3: documentos exigidos pela REGRA (sets/when/priority), owner-aware <<<
  -- Resolve UM set: o de maior priority cujo `when` bate (when null = default). Nunca união.
  v_best_set := null;
  for v_set in
    select value from jsonb_array_elements(coalesce(v_policy.document_rules -> 'sets', '[]'::jsonb)) as t(value)
  ...
  loop
    ...
    if v_when is not null and jsonb_typeof(v_when) = 'object' then
      for v_cond in select value from jsonb_array_elements(coalesce(v_when -> 'all', '[]'::jsonb)) as t(value)
      loop
        select cfd.module into v_fmod from public.custom_field_definitions cfd
          where cfd.id = (v_cond ->> 'field')::uuid and cfd.organization_id = _organization_id;
        v_rec := case when v_fmod in ('opportunities', 'opportunity') then v_op.id else v_op.contact_id end;
        select cfv.value into v_cfv from public.custom_field_values cfv
          where cfv.organization_id = _organization_id
            and cfv.field_definition_id = (v_cond ->> 'field')::uuid
            and cfv.record_id = v_rec;
        if coalesce(v_cond ->> 'op', 'eq') = 'in' then
          v_match := v_match and v_cfv is not null
            and (v_cfv #>> '{}') in (select jsonb_array_elements_text(coalesce(v_cond -> 'value', '[]'::jsonb)));
        else
          v_match := v_match and v_cfv is not null and (v_cfv #>> '{}') = (v_cond ->> 'value');
        end if;
  ...
  if v_best_set is not null then
    for v_code in select value from jsonb_array_elements_text(coalesce(v_best_set -> 'required', '[]'::jsonb)) as t(value)
    loop
      select dt.id, dt.owner_type, dt.name into v_dt
      from public.document_types dt
      where dt.code = v_code and dt.deleted_at is null and dt.is_active
        and (dt.organization_id = _organization_id or dt.organization_id is null)
      order by dt.organization_id nulls last
      limit 1;
      if not found then continue; end if; -- código inexistente: ignora (validação é na escrita)
      ...
      v_ok := v_owner_id is not null and exists (
        ...
        where d.organization_id = _organization_id and d.entity_type = v_dt.owner_type
          and d.entity_id = v_owner_id and d.document_type_id = v_dt.id
      ...
      v_items := v_items || jsonb_build_array(jsonb_build_object(
        'code', 'document:' || v_dt.id::text, 'document_type_id', v_dt.id,
        'label', v_dt.name, 'status', case when v_ok then 'passed' else 'missing' end, 'action', 'edit_documents'));
    end loop;
  end if;
...
-- Shape: novo default + migra as policies existentes (vazio → sem mudança de comportamento).
alter table public.opportunity_close_policies
  alter column document_rules set default '{"version":2,"sets":[]}'::jsonb;

update public.opportunity_close_policies
   set document_rules = jsonb_build_object('version', 2, 'sets', coalesce(document_rules -> 'sets', '[]'::jsonb))
 where coalesce(document_rules ->> 'version', '') <> '2' or (document_rules -> 'sets') is null;

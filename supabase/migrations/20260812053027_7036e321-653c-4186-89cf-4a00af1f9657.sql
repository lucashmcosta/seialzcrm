CREATE OR REPLACE FUNCTION public.sales_thread_status_rank(p_status text)
...
BEGIN
  ...
  IF v_final_assignee IS DISTINCT FROM v_prev_assignee THEN
    INSERT INTO public.thread_assignment_history (thread_id, from_user_id, to_user_id, reason, changed_at)
    VALUES (p_winner, v_prev_assignee, v_final_assignee, 'MERGE_SALES_V2', now());
  END IF;
...
REVOKE ALL ON FUNCTION public.merge_sales_threads(uuid,uuid,uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.merge_sales_threads(uuid,uuid,uuid) FROM anon;
REVOKE ALL ON FUNCTION public.merge_sales_threads(uuid,uuid,uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.merge_sales_threads(uuid,uuid,uuid) TO service_role;
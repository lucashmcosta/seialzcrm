      ...
      if (action === 'sync') {
        ...
        for (const template of twilioTemplates) {
          ...
          try {
            ...
            console.log(`[SYNC-GET] Template ${template.sid} - ApprovalRequests HTTP status: ${approvalResp.status}`)
            if (approvalResp.ok) {
              ...
              if (approvalData.whatsapp) {
                ...
                if (!mappedStatus) {
                  console.warn(`[SYNC-GET] Unknown approval status: "${whatsappStatus}" for ${template.sid} - defaulting to draft`)
                }
              ...
              console.log(`[SYNC-GET] Template ${template.sid} - Final mapped status: ${templateStatus}, category: ${templateCategory}`)
            } else {
              const errorText = await approvalResp.text()
              console.warn(`[SYNC-GET] Template ${template.sid} - ApprovalRequests non-OK response:`, errorText)
            }
          } catch (e: any) {
      ...
      if (postAction === 'sync') {
        ...
        for (const template of twilioTemplates) {
          ...
          try {
            ...
            console.log(`[SYNC-POST] Template ${template.sid} - ApprovalRequests HTTP status: ${approvalResp.status}`)
            if (approvalResp.ok) {
              const approvalData = await approvalResp.json()
              ...
              if (approvalData.whatsapp) {
                ...
                const mappedStatus = statusMap[whatsappStatus]
                if (!mappedStatus) {
                  console.warn(`[SYNC-POST] Unknown approval status: "${whatsappStatus}" for ${template.sid} - defaulting to draft`)
                }
                templateStatus = mappedStatus || 'draft'
                templateCategory = (approvalData.whatsapp.category || 'utility').toLowerCase()
                rejectionReason = approvalData.whatsapp.rejection_reason || null
              }
              console.log(`[SYNC-POST] Template ${template.sid} - Final mapped status: ${templateStatus}, category: ${templateCategory}`)
            } else {
              const errorText = await approvalResp.text()
              console.warn(`[SYNC-POST] Template ${template.sid} - ApprovalRequests non-OK response:`, errorText)
            }
          } catch (e: any) {

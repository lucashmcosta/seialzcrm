...
serve(async (req) => {
  ...
  try {
    const { itemId }: ProcessRequest = await req.json();
    ...
    // Fetch the item with resolved_content
    ...
    // Use resolved_content (materialized by trigger) or fall back to content
    const contentToProcess = item.resolved_content || item.content || item.metadata?.original_content || item.title;
    
    if (!contentToProcess) {
      throw new Error(`No content to process for item: ${itemId}`);
    }

    console.log(`📄 Using ${item.resolved_content ? 'resolved_content' : 'content'} (${contentToProcess.length} chars)`);
    ...
    if (deleteError) {
      console.warn("Warning deleting old chunks:", deleteError.message);
    }

    // Chunk the content
    const chunks = chunkContent(contentToProcess, 1200, 150);
    console.log(`📄 Generated ${chunks.length} chunks from ${contentToProcess.length} characters`);
    ...
    // Update item: mark as published, clear needs_reindex, set last_indexed_at
    ...
    return new Response(
      JSON.stringify({
        ...
        usedResolvedContent: !!item.resolved_content,
      }),

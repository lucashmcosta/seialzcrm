...
serve(async (req) => {
  ...
  try {
    ...
    const resolved = await resolveConnectionWithAsset(admin, organization_id, "ad_account");
    if (!resolved) return json({ error: "no_ad_account" }, 404);
    const connection_id = resolved.connection_id;
    const adAccount = resolved.assets["ad_account"] as string;

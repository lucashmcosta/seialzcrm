-- =====================================================================
-- Inbox v2 — Fase 0 (a): Schema (DDL transacional)
-- NÃO APLICAR ainda. Revisão pendente.
-- Tempo estimado: < 5s. Lock: ACCESS EXCLUSIVE de milissegundos
-- (ADD COLUMN com default constante é metadata-only em PG11+).
-- =====================================================================

begin;

-- ---------- 1. Colunas novas em integration_inbound_events ----------
-- Todas com default constante OU NULL → metadata-only, sem rewrite.
...
alter table public.integration_inbound_events
  ...
  add column if not exists error_classification text,         -- Retryable|Permanent|Conflict
...
create policy "service_role_all_handlers"
  on public.integration_inbound_handlers
  for all
  to service_role
  using (true) with check (true);

-- ---------- 3. Feature flags (org-specific OU global) ----------
...
create policy "service_role_all_flags"
  on public.integration_feature_flags
  for all
  to service_role
  using (true) with check (true);

-- Seed: todas desligadas
...
values
  ...
  ('inbox_v2.write.suvsign',    null, false)   -- gate explícito p/ Fase 2 dry-run → real
...
create policy "service_role_all_dla"
  on public.integration_inbound_dead_letter_archive
  for all to service_role using (true) with check (true);

-- ---------- 5. Dry-run log (Fase 2; criado agora p/ não exigir nova migration) ----------
...
create policy "service_role_all_dryrun"
  on public.integration_inbound_dry_run_log
  for all to service_role using (true) with check (true);

-- ---------- 6. Ingest errors (suporte à Fase 1) ----------
...
create policy "service_role_all_ingest_errors"
  on public.integration_inbound_ingest_errors
  for all to service_role using (true) with check (true);

-- ---------- 7. Estender process_status (adicionar 'expired','claimed','processed_dry_run','dead_letter') ----------
-- process_status é TEXT (não enum) → nada a alterar no schema; valores são validados em RPC.

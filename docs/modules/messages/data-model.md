...
| `messaging_lines` | — | Papel de linha por org (`commercial`, `customer_service`) → `active_endpoint_id` atual. Fonte de verdade do número de envio. **Pré-requisito do vínculo de número**: sem a linha do destino, `sales-route-operations` falha com `SALES_ROUTE_NOT_FOUND` / `CUSTOMER_SERVICE_ROUTE_NOT_FOUND`. Desde 2026-09-22 as duas linhas WhatsApp são semeadas automaticamente pela trigger `trg_seed_default_messaging_lines` (`fn_seed_default_messaging_lines`, `AFTER INSERT ON public.organizations`, à prova de exceção) — vale para qualquer caminho de criação de conta (Portal Admin, signup `handle_new_user`, insert direto). Índice único `uniq_messaging_lines_org_channel_key` em `(organization_id, channel, key)` impede duplicidade; NÃO inserir essas linhas em edge functions. Ver [`plans/2026-07-endpoint-lines-rotation.md`](../../plans/2026-07-endpoint-lines-rotation.md). |
